...
namespace ClassesQuestionnaires
{
   public partial class AjouterQuestion : Form
   {
      ...
      private void BTN_Ajouter_Modifier_Click(object sender, EventArgs e)
      {
         ...
         if (fonction == "AJOUT")
            Ajouter(BRep, MRep1, MRep2, MRep3);
         else
            Modifier(ID, BRep,MRep1, MRep2, MRep3);
      }

      private void Ajouter(String BRep, String MRep1, String MRep2, String MRep3)
      {
         try
         {
      ...
      private void Modifier(String ID, String BRep, String MRep1, String MRep2, String MRep3)
      {
         try
         {            
            OracleCommand oraAjout = new OracleCommand("PKG_GESTION", connection);
            oraAjout.CommandText = "PKG_GESTION.MODIFIERQUESTION";
            oraAjout.CommandType = CommandType.StoredProcedure;
      ...
      private void AfficherReponses(String ID)
      {
         try
         {
            OracleCommand inserer = new OracleCommand("PKG_GESTION", connection);
            inserer.CommandText = "PKG_GESTION.AFFICHERREPONSE";
            inserer.CommandType = CommandType.StoredProcedure;
            ...
            OraDesc.Value = ID;
            OraDesc.Direction = ParameterDirection.Input;
            inserer.Parameters.Add(OraDesc);

            OracleDataReader reader = inserer.ExecuteReader();

            reader.Read();
            TB_Reponse1.Text = reader.GetString(0);
            reader.Read();
            TB_Reponse2.Text = reader.GetString(0);
            reader.Read();
            TB_Reponse3.Text = reader.GetString(0);
            reader.Read();
            TB_Reponse4.Text = reader.GetString(0);
            RB_Reponse1.Checked = true;
         }
         catch (Exception se)
         {
            MessageBox.Show(se.Message.ToString());
